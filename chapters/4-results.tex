...
\textbf{Description} & \textbf{PSNR $\uparrow$} & \textbf{SSIM $\uparrow$} & \textbf{LPIPS $\downarrow$} \\
\hline
Capture data every frame & \cellcolor{green} 23.258400 & 0.723872 & 0.228383 \\
\cellcolor{blue}Capture data every 2$^{\text{nd}}$ frame & 23.251682 & \cellcolor{green} 0.727191 & \cellcolor{green} 0.221351 \\
...
Capture data every 5$^{\text{th}}$ frame & \cellcolor{red} 21.917959 & \cellcolor{red} 0.678348 & \cellcolor{red} 0.258932 \\
\hline
\end{tabular}
\end{table}
...
\section{Experiment 2: Simulated Noise Conditions} \label{sec:gaussian-noise}
...
\begin{table}[ht]
\caption[Results for experiment 4: Naive Block-NeRF on 4-segment dataset]{The average metrics across the four segments compared to the metrics for a single NeRF trained on the same scene. The full overview of each segment's metrics can be viewed in the appendix in \autoref{tab:block-nerf-four-segments-full}.}
...
\begin{table}[ht]
\caption[Results for experiment 4: Naive Block-NeRF on 12-segment dataset]{The average metrics across the twelve segments compared to the metrics for a single NeRF trained on the same scene. The full overview of each segment's metrics can be viewed in the appendix in \autoref{tab:block-nerf-twelve-segments-full}.}
...
\begin{table}[ht]
\caption[Results for experiment 4.1: Removing Artifacts]{Average across different naive Block-NeRF overlap configurations. The overlap becomes less visible with higher overlap values, but it comes at the cost of the previously explored capacity issue.}
...
\begin{table}[ht]
\caption[Results from experiment 5: Real data]{Results from training NeRF on dataset 1. The transformation matrices are approximated with COLMAP or estimated from \acrshort{gps}-readings. "BN" is an abbreviation of the naive Block-NeRF and the resulting metric score is averaged across the 4 NeRFs evaluations.}
